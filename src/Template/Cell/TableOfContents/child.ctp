    ...
    <?php foreach($nodes as $node): ?>
        <li>
            ...
            <ul class="menu-list <?php echo $active ? '' : 'is-hidden'; ?> table-of-contents-list">
                ...
                <li class=""><?php echo $this->Html->link($this->Html->tag('i', '', ['class' => 'fas fa-sticky-note']) . '&nbsp;' . __("Overview"), ['controller' => 'nodes', 'action' => 'view', $node->id], ['class' => '', 'escape' => false]); ?></li>
                ...
                <li class=""><?= $this->Form->postLink($this->Html->tag('i', '', ['class' => 'fas fa-trash']) . '&nbsp;' . __('Delete Node'), ['action' => 'delete', $node->id], ['escape' => false, 'confirm' => __('Are you sure you want to delete # {0}?', $node->id)]) ?></li>
            </ul>
        </li>
    <?php endforeach; ?>
<?php endif; ?>
