...
<div class="nodes view large-9 medium-8 columns content">
    ...
    <nav class="nav nav-pills nav-fill bg-light rounded">
        ...
        <?= $this->Html->link(__('New Object'), ['controller' => 'Objects', 'action' => 'add'], ['class' => 'nav-item btn btn-outline-secondary m-2']) ?>
    ...
    <div class="related">
        ...
        <table cellpadding="0" cellspacing="0" class="table table-sm table-striped table-hover">
            ...
            <tr>
                ...
                <td class="btn-group">
                    <?= $this->Html->link(__('View'), ['controller' => 'Objects', 'action' => 'view', $objects->id], ['class' => 'btn btn-primary']) ?>
                    <?= $this->Html->link(__('Edit'), ['controller' => 'Objects', 'action' => 'edit', $objects->id], ['class' => 'btn btn-warning']) ?>
                    <?= $this->Form->postLink(__('Delete'), ['controller' => 'Objects', 'action' => 'delete', $objects->id], ['confirm' => __('Are you sure you want to delete # {0}?', $objects->id), 'class' => 'btn btn-danger']) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
